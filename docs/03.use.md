...
You can configure multiple Sets for the same model type simultaneously. This can be useful if you need to work with the same model type, but with different endpoints. For example, if you have a WebApi that exposes a collection of books, but you need to work with two different endpoints for this collection, you can configure two Sets for the same model type:
...
You can then retrieve the Set's context by specifying the name of the Set you want to work with.

```csharp
// resolve service implicitly:
